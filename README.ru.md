...
Основная функция данного плагина — создание шаблонов плагинов под разные задачи. С учетом наличия уже готового инструмента [Plugin Builder](https://github.com/g-sherman/Qgis-Plugin-Builder), данное решение позволяет создать плагин практически «под ключ». Конечный результат — сам плагин в виде zip-архива и файлы, которые его составляют.
...
Предположим, что название плагина — `test_plugin`, тип — `Action` и выходной путь — `C:\GIS\plugin_folder`. При установке данного плагина и нажатии на его кнопку в QGIS появится голубая строка уведомления с текстом. Это означает, что все хорошо, плагин работает.
...
<details><summary>⚠️ Удалено в v1.2.0</summary>
    
## Инструмент Kolba


### Колба - это модуль в составе easyPlugin, который был удален в версии 1.2.0. Сейчас он доступен в виде отдельного плагина QGIS с установкой через официальный репозиторий. 
...
> Модули `iface` and `PyQt5` импортированы в шапке скрипта, так как это требуется для корректного запуска из Kolba.

Данный скрипт может быть сохранен как Python-файл (например, `my_widget.py`) и помещен в папку, путь к которой указывается в окне Kolba. Для обновления содержимого папки в Kolba можно нажать кнопку с синими стрелками. Скрипт можно запустить двойным кликом по нему или выбором скрипта с последующим нажатием кнопки со знаком ▶︎.

В правой части окна Kolba расположена информация о запускаемых файлах. Чтобы добавить такую информацию, необходимо создать файл `descriptions.json` в папке, путь к которой указан в окне Kolba. Содержимое этого файла должно иметь следующий вид словаря:
```
{
